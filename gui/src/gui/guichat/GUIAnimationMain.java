...
public class GUIAnimationMain extends JPanel implements ActionListener {
    ...
    private Ellipse2D.Float ellipse = new Ellipse2D.Float(); // 描画されアニメーションされる楕円形

    final int FRAME_SPEED = 50; // アニメーションの速度
    final private double MAX_Counter = 5000; // アニメーションをリセットする最大カウンタ値

    private boolean isResetProcess = true; // 初期化プロセスを実行するかどうかを示すフラグ
    private double counter; // 楕円のサイズとステップを追跡する変数
    private Timer timer; // アニメーションの速度を制御するTimerオブジェクト

    private int INIT_BALLNUM = 3; // アニメーション内の初期ボール数
    private GUIAnimationBall[] myBallRims = new GUIAnimationBall[INIT_BALLNUM]; // GUIAnimationBallオブジェクトの配列

    public static void main(String[] args) {// main 関数
        System.out.println("GUIAnimationFaceObjMain started"); // 開始メッセージを表示
        GUIAnimationMain animation = new GUIAnimationMain(); // GUIAnimationMainのインスタンスを作成
        ...
        // TCPChatサーバをスタート!!!!!
        /***********************/
        // GUIAniMultiTCPServer2のインスタンスを作成し、アニメーションを開始
